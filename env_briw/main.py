        ...
        if ppo:
            ...
            if train:
                model = train_dqn.train_dqn_model(
                    env, learning_rate, total_timesteps, full_path, batch_size, buffer_size, gradient_steps, target_update_interval)
            if plot_train:
            ...
            if eval:
                ...
                eval_dqn.evaluate_policy_and_log_detailed_metrics(
                    model, env, full_path, n_eval_episodes=20)
            ...
            if eval:
                best_bs_path = os.path.join(
                    bs_path, 'pickle_file/best_base_stock.pkl')
                with open(best_bs_path, 'rb') as file:
                    base_stock_level = pickle.load(file)
                bs_class = BS.BSpolicy(base_stock_level)
                eval_BS.evaluate_policy_and_log_detailed_metrics(
                    env, bs_class, bs_path, n_eval_episodes=20)
            if plot_eval:
            ...
            if train:
                ...
                env.seed(42)
                train_sQ.train_sQ_policy(
                    env, 0, 10, 0, 10, sq_path, total_timesteps)
            ...
            if eval:
                ...
                eval_sQ.evaluate_policy_and_log_detailed_metrics(
                    env, sq_class, sq_path, n_eval_episodes=20)
            if plot_eval:
